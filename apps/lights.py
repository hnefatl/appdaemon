...
from typing import Optional, Callable, Any, NewType
...
def get_state(hass: hass.Hass, entity_id: EntityId) -> str:
    state = hass.get_state(entity_id=str(entity_id))
    if state is None:
...
class Room:
    ...
    def make_room(
        ...
        if motion_sensors is None:
            motion_sensors = {f"binary_sensor.{name}_motion_occupancy"}
    ...
    def initialise(self, hass: hass.Hass):
        for motion_sensor in self.motion_sensors:
            hass.listen_state(callback=self.on_room_motion, entity_id=motion_sensor, new="on")
            hass.listen_state(
    ...
    def on_room_motion(self, entity_id: str, *_):
        self.last_motions[entity_id] = datetime.datetime.now()
        ...
        if area_lights_off or not self.lights_on:
            ...
            self._verbose_log(f"motion in {self.name} from {entity_id} but lights already on")
    ...
    def on_activity_sensor_change(self, entity: str, *_):
        ...
        time_since_last_motion = min(now - last_motion for last_motion in self.last_motions.values())
        ...
        ROOMS = [
            ...
            Room.make_room(
                ...
                motion_sensors={
                    ...
                    "binary_sensor.reading_chair_motion_occupancy",
                },
                activity_sensors=[
